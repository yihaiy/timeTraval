...
contract ERC20 is IERC20, ContractOwner {
    ...
    constructor(string memory _name, string memory _symbol,
        uint8 _decimals, uint256 _maxSupply) {                   //构造函数
    ...
    function burn(uint256 amount) external override {      //销毁调用者代币
        _burn(msg.sender, amount);
    }
    
    function burnFrom(address from, uint256 amount) external {                     //代理销毁
