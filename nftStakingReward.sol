/**
 *Submitted for verification at hecoinfo.com on 2021-03-20
*/
...
interface IPromote{
    function newDeposit(address sender) external;
    function redem(address sender) external;
}
...
contract NFT_MasterChef is Ownable {
    ...
    uint256 constant public CUT_PERIOD = 1 hours / 3 seconds;
    ...
    function updatePool(uint256 _pid) public {
        ...
        //     sushiPerBlock = totalWeight.div(5000).mul(sushiPerBlockOrigin);
        // }
        // else{
        //     sushiPerBlock = sushiPerBlockOrigin.mul(10);
        // }
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 sushiReward = multiplier.mul(sushiPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function deposit(uint256 _pid, uint256[] memory tokenIDList) public{
        ...
        if (userInfo[_pid][msg.sender].amount >= 20){
            IPromote(promote).newDeposit(msg.sender);
        }
    }
    ...
    //     //require(userInfo[_pid][msg.sender]._nftBalances.length >0,"not stakeing NFT");
    //     updatePool(_pid);
    //     uint256 amount = withdrawAllNFT(_pid);
    ...
    function withdraw_in(uint256 _pid, uint256 _amount) internal {
        ...
        if (user.amount < 20) {
            IPromote(promote).redem(msg.sender);
        }
        user.rewardDebt = user.amount.mul(pool.accSushiPerShare).div(1e12);
    ...
    function getReward(uint256 _pid) public payable{
        ...
        require(msg.value == bnbAmount,"Fee wrong!");
    ...
    function withdrawNFT(uint256 _pid,uint256 tokenID) internal returns(uint256 totalAmount){
        ...
        require(block.timestamp.sub(lockedTime[tokenID]).mod(lockTime) >= fifteenDays,"NO Enlough Time to lock");  
        (,,uint256 Grade,,,)= INFT(pool.lpToken).starAttributes(tokenID); 
        totalWeight -= nftweight[Grade - 1];
