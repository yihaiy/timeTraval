...
contract Promote is Member {
    ...
    uint256[] internal amountThreshould = [1000*1e18, 2000*1e18, 5000*1e18, 10000*1e18];
    ...
    function bind(address binding) public {
        ...
        require(msg.sender != binding, "can not bing self");
    ...
    function redem(address sender) external onlyPool {
    ...
    function evoDown(address _user) internal {
        ...
        if ( level > 0 && userInfo[_user].num -1 < numThreshold[level - 1]) {
            ...
            if (userInfo[_user].level > 0) {
                ...
                daliyInfo[round].perValidNum[userInfo[_user].level - 1] += (userInfo[_user].num-1);
            }
            emit LevelChange(_user, level, level - 1);
        }
        if( level > 0 && level == userInfo[_user].level) {
            daliyInfo[round].perValidNum[userInfo[_user].level - 1]--;
        }
        userInfo[_user].num--;
    }
    
    function newDeposit(address sender) external onlyPool {
        require(isGamer[sender] == true, "origin must in game!");
        UserInfo storage user = userInfo[sender];
        if(!user.isValid) {
            user.isValid = true;
    ...
    function evo(address _user) internal {
        ...
        if (level != userInfo[_user].level) {
            daliyInfo[round].perValidNum[userInfo[_user].level - 1] += userInfo[_user].num;
            if (level > 0) {
                daliyInfo[round].perValidNum[level - 1] -= (userInfo[_user].num - 1);
            }
        }
        if (level == userInfo[_user].level && level > 0){
    ...
    function withdraw() public {
        ...
        IERC20(era).transfer(msg.sender, userInfo[msg.sender].depositedERA);
    ...
    function pendingRewards(address _user) public view returns (uint256 reward){
        ...
        for(i; i >0; i--) {
            ...
            mul(num).div(daliyInfo[round-i].perValidNum[level-1]));
        }
